...
function buildWorkflowSection(sandboxMode: boolean): string[] {
  const steps = [
    ...
    '   - For binary files (images, fonts, etc.): use `shell_exec` with commands like:',
    '     - `git clone <repo-url> /tmp/<name>` to fetch files from external repos',
    '     - `cp /tmp/<name>/path/to/file ./target/path/` to place files',
    '     - `curl -L -o ./target/path/file <url>` to download from URLs',
    '   - Note: `read_file` and `write_file` are text-only; use shell commands for binary files',
    '',
    '4. **Verify** (Required)',
  ...
  if (sandboxMode) {
    steps.push(
      ...
      '     2. `git_commit` with a structured commit message in English',
      '   - Use conventional commit format: `feat:`, `fix:`, `refactor:`, etc.',
      '   - Commit message MUST include:',
      '     - **First line**: conventional commit subject (e.g. `feat: add dark mode toggle`)',
      '     - **Body** (after blank line):',
      '       - `Requested-by: <name>` — the person who requested this change (from task description)',
      '       - A brief summary of the problem/requirement being addressed',
      '   - Example:',
      '     ```',
      '     feat: add export button to report page',
      '',
      '     Requested-by: Alice',
      '     Add PDF export functionality to the monthly report page.',
      '     Users need to download reports for offline review.',
      '     ```',
      '',
  ...
  steps.push(
    ...
    '     - `thinking`: Detailed explanation of your approach, including:',
    '       - What was the problem/requirement',
    '       - How you analyzed and located the relevant code',
