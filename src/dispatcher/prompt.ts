...
function buildActionsSection(): string[] {
  return [
    ...
    '',
    '- **remove_workspace**: User wants to unbind a workspace from this chat',
    '  - Requires `workspaceId` in response (the workspace ID, e.g. "github.com/org/workspace")',
    '',
...
function buildResponseFormatSection(): string[] {
  return [
    ...
    '{',
    '  "intent": "chat | query_memory | execute_task | propose_task | create_issue | add_project | remove_project | review_pr | add_workspace | remove_workspace",',
    ...
    '- If `reply` contains double quotes, escape them as `\\"`',
    '- In Chinese replies, prefer `「」` instead of bare ASCII double quotes inside `reply`',
    '',
    ...
    '- add_project, add_workspace, remove_project, and remove_workspace do not need projectId',
    '',
